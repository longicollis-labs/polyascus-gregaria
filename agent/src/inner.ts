...
import {wornConstruction, wornPattern, surfaceForm, STALE_CUE} from "./monoculture.js";
...
// The worn-construction scan (wornConstruction/wornPattern/surfaceForm/STALE_CUE)
// now lives in monoculture.ts, shared with the language-currents evolve; the only
// difference is it takes the already-flattened fields, so we pass stateFields(s).
...
export async function evolveInnerState(input: {
    ...
    const read = readingWith(input.stage, 2);
    ...
    const wornStem = wornConstruction(stateFields(input.current));
    const pat = wornStem ? wornPattern(wornStem) : null;
    const worn = pat ? surfaceForm(stateFields(input.current), pat) || wornStem : null;
